<?php
require_once(dirname( __FILE__ ) . '/OpayModuleHelper.php');

class OpayPaymentHelper extends OpayModuleHelper
{
  /**
  ...
  /**
    * @var string SDK class name(required)
    */
   protected $sdkClassName = 'OpayWooAllInOne';

   /**
    * @var string SDK file path(required)
    */
   protected $sdkFilePath = 'Opay.Payment.Integration.Shell.php';

   /**
    * @var string Service provider
    */
   private $provider = 'OPay';
   ...
   /**
    * OpayPaymentHelper constructor.
    */
   public function __construct()
   {
       parent::__construct();
       $this->encryptType = OpayEncryptType::ENC_SHA256;
       $this->setStageMerchantIds(array('2000132', '2000214'));
       $this->serviceUrls['prod'] = 'https://payment.'. $this->prefix .'.tw';
       $this->serviceUrls['stage'] = 'https://payment-stage.'. $this->prefix .'.tw';
   }
   ...
   private function checkoutPrepare($data)
   {
       ...
       switch ($this->sdk->Send['ChoosePayment']) {
            ...
            case $this->getSdkPaymentMethod('topupused'):
            case $this->getSdkPaymentMethod('weixinpay'):
                break;
            default:
   ...
   private function getSdkPaymentMethod($paymentType = '')
   {
       ...
       switch ($lower) {
           case 'all':
               $sdkPayment = OpayPaymentMethod::ALL;
               break;
           case 'credit':
               $sdkPayment = OpayPaymentMethod::Credit;
               break;
           case 'webatm':
               $sdkPayment = OpayPaymentMethod::WebATM;
               break;
           case 'atm':
               $sdkPayment = OpayPaymentMethod::ATM;
               break;
           case 'cvs':
               $sdkPayment = OpayPaymentMethod::CVS;
               break;
           case 'topupused':
               $sdkPayment = OpayPaymentMethod::TopUpUsed;
               break;
           case 'weixinpay':
               $sdkPayment = OpayPaymentMethod::WeiXinpay;
               break;
           default:
               $sdkPayment = '';
               break;
   ...
   {
       if ($type === 'Y') {
           return OpayExtraPaymentInfo::Yes;
       }
       return OpayExtraPaymentInfo::No;
   }
