 ...
 * 付款方式。
 */
abstract class OpayPaymentMethod {
 ...
 * 付款方式子項目。
 */
abstract class OpayPaymentMethodItem {
 ...
 * 額外付款資訊。
 */
abstract class OpayExtraPaymentInfo {
 ...
 * 電子發票開立註記。
 */
abstract class OpayInvoiceState {
    /**
     * 需要開立電子發票。
     */
    const Yes = 'Y';
...
abstract class OpayEncryptType {
    // MD5(預設)
    const ENC_MD5 = 0;
    ...
    */
    ...
    function __construct() {
        ...
        $this->Send = array(
            ...
            "TotalAmount"       => '',
            "TradeDesc"         => '',
            "ChoosePayment"     => OpayPaymentMethod::ALL,
            "Remark"            => '',
            "ChooseSubPayment"  => OpayPaymentMethodItem::None,
            "NeedExtraPaidInfo" => OpayExtraPaymentInfo::No,
            "DeviceSource"      => '',
            "IgnorePayment"     => '',
            "PlatformID"        => '',
            "InvoiceMark"       => OpayInvoiceState::No,
            "Items"             => array(),
            "UseRedeem"         => OpayUseRedeem::No,
            "HoldTradeAMT"      => 0,
            "StoreID"           => ''
        );
    ...
    function CheckOut($target = "_self") {
        ...
        OpaySend::CheckOut($target,$arParameters,$this->SendExtend,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //產生訂單html code
    function CheckOutString($paymentButton = null, $target = "_self") {
        $arParameters = array_merge( array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType) ,$this->Send);
        return OpaySend::CheckOutString($paymentButton,$target = "_self",$arParameters,$this->SendExtend,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //取得付款結果通知的方法
    function CheckOutFeedback() {
        return $arFeedback = OpayCheckOutFeedback::CheckOut(array_merge($_POST, array('EncryptType' => $this->EncryptType)),$this->HashKey,$this->HashIV,0);
    }

    //訂單查詢作業
    function QueryTradeInfo() {
        return $arFeedback = OpayQueryTradeInfo::CheckOut(array_merge($this->Query,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL) ;
    }

    //信用卡定期定額訂單查詢的方法
    function QueryPeriodCreditCardTradeInfo() {
        return $arFeedback = OpayQueryPeriodCreditCardTradeInfo::CheckOut(array_merge($this->Query,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }

    //信用卡關帳/退刷/取消/放棄的方法
    function DoAction() {
        return $arFeedback = OpayDoAction::CheckOut(array_merge($this->Action,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }

    //廠商通知退款
    function AioChargeback() {
        return $arFeedback = OpayAioChargeback::CheckOut(array_merge($this->ChargeBack,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }

    //會員申請撥款／退款
    function AioCapture(){
        return $arFeedback = OpayAioCapture::Capture(array_merge($this->Capture,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }
    ...
    function TradeNoAio($target = "_self"){
        ...
        OpayTradeNoAio::CheckOut($target,$arParameters,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //查詢信用卡單筆明細紀錄
    function QueryTrade(){
        return $arFeedback = OpayQueryTrade::CheckOut(array_merge($this->Trade,array("MerchantID" => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }
    ...
    function FundingReconDetail($target = "_self"){
        ...
        OpayFundingReconDetail::CheckOut($target,$arParameters,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }
}
...
*  產生訂單
*/
class OpaySend extends OpayAio
{
    //付款方式物件
    ...
    {
        //宣告付款方式物件
        $PaymentMethod    = 'Opay'.$arParameters['ChoosePayment'];
    ...
    static function CheckOut($target = "_self",$arParameters = array(),$arExtend = array(),$HashKey='',$HashIV='',$ServiceURL=''){
        ...
        //產生檢查碼
        $szCheckMacValue = OpayCheckMacValue::generate($arParameters,$HashKey,$HashIV,$arParameters["EncryptType"]);
        ...
        $szHtml .=     '</head>';
        $szHtml .=     '<body>';
        $szHtml .=         "<form id=\"__opayForm\" method=\"post\" target=\"{$target}\" action=\"{$ServiceURL}\">";
        ...
        $szHtml .=         '</form>';
        $szHtml .=         '<script type="text/javascript">document.getElementById("__opayForm").submit();</script>';
        $szHtml .=     '</body>';
        $szHtml .= '</html>';
...
class OpayCheckOutFeedback extends OpayAio
{
    static function CheckOut($arParameters = array(),$HashKey = '' ,$HashIV = ''){
        ...
        $CheckMacValue = OpayCheckMacValue::generate($arParameters,$HashKey,$HashIV,$EncryptType);
...
class OpayQueryTradeInfo extends OpayAio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        ...
        if (sizeof($arErrors) == 0) {
            ...
            if (sizeof($arFeedback) > 0) {
                $szConfirmMacValue = OpayCheckMacValue::generate($arConfirmArgs,$HashKey,$HashIV,$EncryptType);
...
class OpayQueryPeriodCreditCardTradeInfo extends OpayAio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        $arErrors = array();
        $arParameters['TimeStamp'] = time();
...
class OpayDoAction extends OpayAio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        // 變數宣告。
        $arErrors = array();
...
class OpayAioChargeback extends OpayAio
{
    static function CheckOut($arParameters = array(), $HashKey='',$HashIV='',$ServiceURL=''){
...
class OpayAioCapture extends OpayAio
{
...
class OpayTradeNoAio extends OpayAio
{
...
class OpayQueryTrade extends OpayAio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        $arErrors = array();
        $arFeedback = array();
...
class OpayFundingReconDetail extends OpayAio
{
    static function CheckOut($target = "_self",$arParameters = array(),$HashKey='',$HashIV='',$ServiceURL=''){
        //產生檢查碼
        $EncryptType = $arParameters['EncryptType'];
...
abstract class OpayVerification
{
    // 付款方式延伸參數
    ...
    public function check_invoiceString($arExtend = array()){
        ...
        if(!array_key_exists('CarruerType', $arExtend)){
            $arExtend['CarruerType'] = OpayCarruerType::None ;
        }else{
            //有設定統一編號的話，載具類別不可為合作特店載具或自然人憑證載具。
            $notPrint = array(OpayCarruerType::Member, OpayCarruerType::Citizen);
        ...
        if(!array_key_exists('CustomerID', $arExtend)) {
            ...
            if($arExtend['CarruerType'] == OpayCarruerType::Member && strlen($arExtend['CustomerID']) == 0){
                array_push($arErrors, "CustomerID is required.");
            }
        }
        // 捐贈註記 Donation(預設為No)
        if(!array_key_exists('Donation', $arExtend)){
            $arExtend['Donation'] = OpayDonation::No ;
        }else{
            //若有帶統一編號，不可捐贈
            if(strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Donation'] != OpayDonation::No  ){
        ...
        if(!array_key_exists('Print', $arExtend)){
            $arExtend['Print'] = OpayPrintMark::No ;
        }else{
            //捐贈註記為捐贈(Yes)時，請設定不列印(No)
            if($arExtend['Donation'] == OpayDonation::Yes && $arExtend['Print'] != OpayPrintMark::No){
                array_push($arErrors, "Print should be No.");
            }
            // 統一編號不為空字串時，請設定列印(Yes)
            if(strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Print'] != OpayPrintMark::Yes){
        ...
        if(!array_key_exists('CustomerName', $arExtend)){
            ...
            if($arExtend['Print'] == OpayPrintMark::Yes && strlen($arExtend['CustomerName']) == 0){
        ...
        if(!array_key_exists('CustomerAddr', $arExtend)){
            ...
            // 列印註記為列印(Yes)時，此參數不可為空字串
            if($arExtend['Print'] == OpayPrintMark::Yes && strlen($arExtend['CustomerAddr']) == 0){
        ...
        if(!array_key_exists('ClearanceMark', $arExtend)) {
            ...
            if($arExtend['TaxType'] == OpayTaxType::Zero && ($arExtend['ClearanceMark'] != OpayClearanceMark::Yes || $arExtend['ClearanceMark'] != OpayClearanceMark::No)) {
                array_push($arErrors, "ClearanceMark is required.");
            }
            if (strlen($arExtend['ClearanceMark']) > 0 && $arExtend['TaxType'] != OpayTaxType::Zero) {
        ...
        if (!array_key_exists('CarruerNum', $arExtend)) {
            ...
            switch ($arExtend['CarruerType']) {
                ...
                case OpayCarruerType::None:
                case OpayCarruerType::Member:
                break;
                // 載具類別為買受人自然人憑證(Citizen)時，請設定自然人憑證號碼，前2碼為大小寫英文，後14碼為數字
                case OpayCarruerType::Citizen:
                ...
                case OpayCarruerType::Cellphone:
        ...
        if ($arExtend['Donation'] == OpayDonation::Yes) {
        ...
        if(!array_key_exists('InvoiceItems', $arExtend)){
            ...
            if ($arExtend['TaxType'] == OpayTaxType::Mix) {
                if (in_array(OpayTaxType::Dutiable, $tmpItemTaxType) and in_array(OpayTaxType::Free, $tmpItemTaxType)) {
                    // Do nothing
                }  else {
                    $tmpItemTaxType = array();
                }
...
*  付款方式：超商代碼
*/
class OpayCVS extends OpayVerification
{
    public  $arPayMentExtend = array(
                            'Desc_1'           =>'',
                            'Desc_2'           =>'',
...
*  付款方式 ATM
*/
class OpayATM extends OpayVerification
{
    public  $arPayMentExtend = array(
                            'ExpireDate'       => 3,
                            'PaymentInfoURL'   => '',
...
*  付款方式 WebATM
*/
class OpayWebATM extends OpayVerification
{
    public  $arPayMentExtend = array();
...
* 付款方式：AccountLink
*/
class OpayAccountLink extends OpayVerification
{
    public  $arPayMentExtend = array();
...
* 付款方式 : 信用卡
*/
class OpayCredit extends OpayVerification
{
    public $arPayMentExtend = array(
                                    "CreditInstallment" => '',
                                    "Redeem"            => FALSE,
...
* 付款方式:TopUpUsed
*/
class OpayTopUpUsed extends OpayVerification
{
    public  $arPayMentExtend = array();
...
* 付款方式 : 微信支付
*/
class OpayWeiXinpay extends OpayVerification
{
    public $arPayMentExtend = array();
...
*  付款方式：全功能
*/
class OpayALL extends OpayVerification
{
    public  $arPayMentExtend = array();
...
class OpayCheckMacValue
{
    ...
    static function generate($arParameters = array(),$HashKey = '' ,$HashIV = '',$encType = 0){
        ...
        if(isset($arParameters))
        {
            ...
            uksort($arParameters, array('OpayCheckMacValue','merchantSort'));
            ...
            // 編碼
            switch ($encType) {
                case OpayEncryptType::ENC_SHA256:
                    // SHA256 編碼
                    $sMacValue = hash('sha256', $sMacValue);
                break;

                case OpayEncryptType::ENC_MD5:
                default:
                // MD5 編碼
